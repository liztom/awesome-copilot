...
> **Breaking change — non-interactive plugin commands split by kind (v1.0.85+)**: `copilot plugins list --kind instruction` and `copilot plugins list --kind lsp` are replaced by dedicated `copilot instruction list` and `copilot lsp list` commands. Likewise, `copilot plugins install --skill [--scope project]` is replaced by `copilot skill add [--project]` (the `--scope` spelling is gone), and the cross-kind `--kind`, `--scope`, `--mcp`, and `--skill` flags have been removed from `copilot plugins` entirely — use `copilot mcp` and `copilot skill` instead. Enabling or disabling a component from the command line also moved: `copilot plugins enable/disable --plugin|--mcp|--skill` is replaced by `enable` and `disable` subcommands on each dedicated command, for example `copilot plugin enable my-plugin`, `copilot mcp disable my-server`, or `copilot skill enable my-skill`. All of these commands also support `--json` for scripting: `copilot plugin list --json`, `copilot plugin marketplace list --json`, and `copilot plugin marketplace browse --json`.

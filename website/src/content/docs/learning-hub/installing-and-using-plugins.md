...
> **CLI subcommand renames (v1.0.85+)**: `copilot plugins enable`/`disable --plugin|--mcp|--skill` are replaced by dedicated `enable` and `disable` subcommands on `copilot plugin`, `copilot mcp`, and `copilot skill`. Similarly, `copilot instruction list` and `copilot lsp list` replace `copilot plugins list --kind instruction` and `--kind lsp`:
>
> ```bash
...
> ```
>
> `--json` output is also available on `copilot plugin list`, `copilot plugin marketplace list`, and `copilot plugin marketplace browse` for scripting and automation.

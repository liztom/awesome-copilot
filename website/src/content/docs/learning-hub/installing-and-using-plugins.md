...
> **Unified enable/disable commands (v1.0.85+)**: `copilot plugin`, `copilot mcp`, and `copilot skill` each gained top-level `enable` and `disable` subcommands, replacing `copilot plugins enable/disable --plugin|--mcp|--skill`:
>
> ```bash
...
> ```
>
> The same release added `copilot instruction list` and `copilot lsp list`, replacing `copilot plugins list --kind instruction` and `--kind lsp`, and added a `--json` flag to `copilot plugin list`, `copilot plugin marketplace list`, and `copilot plugin marketplace browse` for scripting and automation.

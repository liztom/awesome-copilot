...
*(v1.0.83+)* Plugin list commands and `/plugin` now show **bundled built-in plugins** alongside marketplace and direct installs. MCP servers contributed by a plugin are no longer mislabeled "User" in the MCP dashboard — a server from a bundled plugin is shown as built-in and names the plugin it came from, and MCP tools stay callable even after an MCP server restarts.

...
> *(v1.0.83+)* `model` can list several models in order of preference (e.g., `model: [Claude Sonnet 4.5, GPT-5]`); Copilot CLI tries each one in turn until it finds a model you have access to. Pair this with `model-policy: required` to keep any future model switches restricted to that list, even if the user changes their global model elsewhere in the session:
>
> ```yaml
> ---
> name: 'Security Reviewer'

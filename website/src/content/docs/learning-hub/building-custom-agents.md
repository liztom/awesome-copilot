...
**Model fallback lists** *(v1.0.83+)*: `model` can also be an array of model names, tried in order until one is available to you. This is useful when your preferred model is temporarily rate-limited, in preview, or not enabled for your account:
...
Pair this with **model-policy: required** to keep model changes restricted to that list—this prevents a user (or the CLI's own fallback logic) from silently switching to a model outside your approved set:

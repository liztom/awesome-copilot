...
> **Configuration sidebar and Vim mode (v1.0.85+)**: Run `/config` to open a sidebar configuration screen without leaving your session — a faster alternative to editing `config.json` by hand for common settings. Vim mode is also available to everyone: turn it on with `/vim`, or set `editorMode` to `vim` in your config for modal editing in the composer. The current editor mode is shown while you type.

> **Unified plugin management commands (v1.0.85+)**: `copilot plugin`, `copilot mcp`, and `copilot skill` now each accept `enable` and `disable` subcommands directly (e.g., `copilot skill disable my-skill`), replacing the older cross-kind `copilot plugins enable/disable --plugin|--mcp|--skill` syntax. Likewise, `copilot instruction list` and `copilot lsp list` replace `copilot plugins list --kind instruction` and `--kind lsp`. The legacy `copilot plugins list --scope` flag has also been retired — use `copilot skill add [--project]` instead of `copilot plugins install --skill [--scope project]`.

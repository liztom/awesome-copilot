...
> **Vim mode for everyone (v1.0.84+)**: Vim mode is no longer experimental. Toggle it on with `/vim` or set `editorMode` to `vim` in your config for modal editing in the composer — the current mode is shown while you type.

...
**`/config` sidebar** *(v1.0.85+)*: `/config` opens configuration in a persistent sidebar panel instead of a full-screen dialog, so you can adjust settings while keeping your conversation visible alongside it:

```
/config
```

**Vim mode** *(v1.0.85+)*: Turn on modal editing in the composer with `/vim`, or set `"editorMode": "vim"` in your user settings to enable it by default. The current mode (Normal/Insert) is shown while you type, matching familiar Vim keybindings for navigating and editing prompts:

```
/vim
```

**Concise transcript view** *(v1.0.85+)*: Set `"transcriptView": "concise"` in `/settings` to group tool activity into expandable work summaries instead of showing every tool call inline. This keeps the conversation view focused on the agent's reasoning and results, with tool details available on demand.

**Context management tools for subagents** *(v1.0.85+)*: Opt in to context management tools for agents and subagents from `/settings`. This lets subagents actively manage their own context window (for example, summarizing or discarding stale context) rather than relying solely on the parent session's compaction behavior.
...
**Session and memory import** *(v1.0.85+)*: Import sessions and memory from a semantic JSONL interchange format, letting you bring conversation history or memory data generated outside the CLI into a session for reference.
...
**`worktreePathTemplate` setting** *(v1.0.87+)*: Controls where `/worktree`, `/move`, `/new`, and `--worktree` create worktrees, using a path template with `{repoPath}`, `{repo}`, `{branch}`, and `{branchSlug}` placeholders. For example, set it to `~/src/worktrees/{repo}/{branch}` to keep worktrees outside your repository checkout entirely. Leave it unset to keep the default `<repo>.worktrees/` layout, where slashes in the branch name are flattened to dashes.
...
**Network host allow/deny rules** *(v1.0.85+)*: `/sandbox` supports fine-grained network host allow and deny rules on top of your configured upstream proxy, so you can permit or block specific hosts without replacing your proxy setup entirely. Use this when a sandboxed task needs access to one additional external service but you don't want to open up broader network access.

**Auto routing tier startup defaults** *(v1.0.87+)*: Organizations can set user and managed startup defaults for the Auto model's routing tier (Efficiency, Balance, or Intelligence), including a strict, non-overridable policy or a user-overridable default. This lets enterprise admins standardize cost/quality tradeoffs for Auto mode across their teams while still allowing individual overrides where policy permits.

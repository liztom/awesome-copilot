...
**Streamer mode** *(v1.0.85+)*: Turning on Streamer mode masks internal model names in `/model`, the footer, and startup diagnostics — useful when screen-sharing or streaming a session and you don't want to reveal which underlying model you're using. Toggling it does not restart model initialization, so it can be switched mid-session without interrupting the current turn.
...
**`/config`** *(v1.0.85+)*: Opens a sidebar configuration screen inside the CLI, giving you a persistent panel view of settings alongside your active conversation instead of a modal dialog. This is a complementary entry point to `/settings` for users who prefer keeping configuration visible while they work.

**Vim mode** *(v1.0.85+)*: Modal editing is available to everyone in the message composer. Turn it on with `/vim`, or set `editorMode` to `vim` in your settings to make it the default for new sessions. The current mode (insert/normal) is shown while you type, matching the muscle memory of Vim users navigating and editing text.

**Concise transcript view** *(v1.0.85+)*: Set `transcriptView` to `"concise"` in `/settings` to group tool activity into expandable work summaries instead of showing every individual tool call inline. This keeps long sessions with many tool calls easier to scan — expand a summary only when you need to see the underlying detail.

**Context management tools for subagents** *(v1.0.85+)*: `/settings` now includes an opt-in for context management tools that agents and subagents can use to manage their own context window more actively during a session.
...
**Sandbox network host allow/deny rules** *(v1.0.85+)*: `/sandbox` supports per-host allow and deny rules that layer on top of your configured upstream proxy, instead of replacing it — grant or block specific hosts without having to reconfigure the whole proxy. `/sandbox policy` also now reports local-network access using your currently configured setting (v1.0.86+), and managed sandbox sessions can be disabled for the rest of the session directly from an approved bypass prompt (v1.0.85+).

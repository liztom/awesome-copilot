...
*(v1.1.18+)* You can edit issue and pull request titles and descriptions, and edit, delete, or hide comments, directly from the app — with an edited indicator shown on changed comments. Pull request fix buttons also gained a **"Fix with instructions"** option so you can add guidance before Copilot runs a fix, instead of only triggering a default fix.

### Files Tab

*(v1.1.17+)* The **Files tab** includes a source switcher for browsing generated artifacts and added file/folder sources without leaving the tab, keeping your selection as you switch between sources. Generated artifacts like Markdown files now open in the Files tab alongside repository files, with an option to promote an artifact into the repository (v1.1.21+).

...
- **Microsoft Entra ID (Azure AD)**: MCP servers that authenticate via Microsoft Entra ID are fully supported. Once you complete the initial login, the CLI caches the authentication and **will not show the consent screen on subsequent connections** — you authenticate once per session rather than every time the server reconnects. On Windows, sign-in for Entra-protected remote servers can also go through the OS authentication broker (WAM) *(v1.0.81+)*, usually with no browser prompt at all; other platforms and machines without the broker library keep the existing browser flow.
- **Client ID Metadata Document (CIMD)** *(v1.0.83+)*: MCP servers that support CIMD-based OAuth sign-in can register a client dynamically from a hosted metadata document instead of requiring a pre-registered client ID, simplifying setup for servers that support this emerging OAuth extension.

...
> **Client ID Metadata Document (CIMD) support** *(v1.0.83+)*: Copilot CLI can authenticate to MCP servers that rely on Client ID Metadata Documents for OAuth sign-in, an emerging alternative to Dynamic Client Registration where the client's identity and metadata are published at a URL instead of registered ahead of time with the authorization server.
